...
@section('title', 'Tambah Pengajuan KRS')
...
<div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-3 sm:gap-4">
            ...
            <div>
                <h2 class="text-xl sm:text-2xl md:text-3xl font-bold text-white">
                    <i> </i> Form Pengajuan KRS Baru
                </h2>
                <p class="text-white/70 text-sm sm:text-base mt-1">Isi data pengajuan rencana studi semester ini</p>
            </div>
        </div>
    </div>

    <!-- Card Form - Modern Glassmorphism -->
    <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div class="p-4 sm:p-6 md:p-8">
            <form action="{{ route('krs.store') }}" method="POST" class="space-y-4 sm:space-y-5 md:space-y-6">
                @csrf

                <!-- Grid Layout untuk Form (Responsive) -->
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <textarea name="courses_list" 
                                  ...
                                  class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200 @error('courses_list') border-red-500 @enderror">{{ old('courses_list') }}</textarea>
                    </div>
    ...
    <div class="mt-4 sm:mt-6 bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4">
        <div class="flex items-start gap-2 text-white/80 text-xs sm:text-sm">
            ...
            <span>Data yang Anda masukkan akan diproses oleh Dosen Pembimbing Akademik. Pastikan data sudah benar sebelum menyimpan.</span>
        </div>
    </div>
</div>
